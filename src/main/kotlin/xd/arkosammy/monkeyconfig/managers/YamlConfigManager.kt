...
internal class YamlConfigManager : AbstractConfigManager {
    ...
    ) : super(configName, settingGroups, YamlFormat.defaultInstance(), FabricLoader.getInstance().configDir.resolve("$configName.yaml"))

    constructor(
        configName: String,
        settingGroups: List<MutableSettingGroup>? = null,
        settingBuilders: List<ConfigSetting.Builder<*, *, *>>
    ) : super(configName, settingGroups, settingBuilders, YamlFormat.defaultInstance(), FabricLoader.getInstance().configDir.resolve("$configName.yaml"))
