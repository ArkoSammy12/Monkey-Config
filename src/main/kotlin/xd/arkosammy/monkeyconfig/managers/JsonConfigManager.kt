...
open class JsonConfigManager : AbstractConfigManager {
    ...
    ) : super(configName, settingGroups, JsonFormat.fancyInstance(), FabricLoader.getInstance().configDir.resolve("$configName.json"))

    constructor(
        configName: String,
        settingGroups: List<MutableSettingGroup>? = null,
        settingBuilders: List<ConfigSetting.Builder<*, *, *>>
    ) : super(configName, settingGroups, settingBuilders, JsonFormat.fancyInstance(), FabricLoader.getInstance().configDir.resolve("$configName.json"))
