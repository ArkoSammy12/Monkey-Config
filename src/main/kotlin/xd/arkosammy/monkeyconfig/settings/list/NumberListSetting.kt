...
open class NumberListSetting<T : Number> @JvmOverloads constructor(
...
) : ListSetting<T, NumberType<T>>(settingLocation, comment, defaultValue, value) {
    ...
    open class Builder<T : Number> @JvmOverloads constructor(
        ...
        comment: String? = null,
        defaultValue: List<T>
    ) : ListSetting.Builder<T, NumberType<T>>(settingLocation, comment, defaultValue) {

        override fun build(): NumberListSetting<T> =
            NumberListSetting(this.settingLocation, this.comment, this.defaultValue)
